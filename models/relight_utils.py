...
def get_visibility_and_indirect_light(  visibility_net, 
    ...
    for chunk_idx in chunk_idxs_vis_compute:
        ...
        nerv_vis_chunk, nerfactor_vis_chunk, indirect_light_chunk = compute_radiance(TensoLight=TensoLight, 
    ...
    for chunk_idx in chunk_idxs_vis_recompute:
        chunk_surface_pts = surface_pts_masked[chunk_idx]  # [chunk_size, 3]
        chunk_surf2light = surf2light_masked[chunk_idx]    # [chunk_size, 3]
        nerv_vis, nerfactor_vis = compute_transmittance(TensoLight=TensoLight, 
                                                        surf_pts=chunk_surface_pts, 
                                                        light_in_dir=chunk_surf2light, 
                                                        nSample=nSample, 
                                                        vis_near=vis_near, 
...
@torch.no_grad()
def compute_visibility(TensoLight, pts, light_xyz, nSample, vis_near, vis_far, args, device='cuda'):
...
@torch.no_grad()
def compute_transmittance(TensoLight, surf_pts, light_in_dir, nSample=128, vis_near=0.1, vis_far=2, device='cuda'):
    '''same way as in NeRV
    - args:
        - TensoLight: base model
    ...
    if ray_valid.any():
        ...
        sigma_feature = TensoLight.compute_densityfeature(xyz_sampled[ray_valid])  # [..., 1] # detach unremoved
        validsigma = TensoLight.feature2density(sigma_feature)
        sigma[ray_valid] = validsigma


    alpha, weight, transimittance = raw2alpha(sigma, dists * TensoLight.distance_scale)
...
@torch.no_grad()
def compute_visibility_and_indirect_light(TensoLight, pts, light_xyz, light_idx, nSample, vis_near, vis_far, args, device='cuda'):
